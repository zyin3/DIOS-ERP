...
# expense type view
# This view is for Get functionality.
@csrf_protect
@login_required
def expense_type_view(request):
    ...
    for item in expensePageList.object_list:
        item_dict = {
          "id":item.pk,
          "expenseType": item.typename.title(),
...
# expense type view
# This view is for edit/del/add functionality.
@csrf_protect
@login_required
def edit_expense_type_view(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('oper') == 'del':
            ...
            ExpenseType.objects.get(id__iexact=del_ID).delete();
            return HttpResponse()
